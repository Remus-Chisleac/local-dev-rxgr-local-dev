...
(function () {
  ...
  // Quick-add trigger aria label ({title} placeholder) — from the modal's
  // config blob (snippets/quick-add-modal). Blob absent → no quick-add on
  // this page, so JS-built cards skip the button like the SSR ones.
  var quickAddOpenAria = '';
  var quickAddEnabled = false;
  (function readQuickAddConfig() {
    var node = document.getElementById('aico-quick-add-config');
    if (!node) {
      return;
    }
    ...
    try {
      ...
      quickAddOpenAria = (parsed.i18n && parsed.i18n.open_aria) || '';
  ...
  function buildCard(hit, eager) {
    ...
    // Quick add-to-cart trigger — mirrors snippets/product-card.liquid: a
    // SIBLING of the card link (the whole card is an <a>), only when the
    // page carries the quick-add modal and the product is purchasable
    // (priced; the listing scope filter already limits hits to in-stock).
    if (quickAddEnabled && priceLabel && urlHandle) {
      var quickAdd = document.createElement('button');
      quickAdd.type = 'button';
      quickAdd.className = 'aico-quick-add-trigger';
      quickAdd.setAttribute('data-aico-quick-add', '');
