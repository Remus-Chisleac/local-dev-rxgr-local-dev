...
(function () {
  ...
  function syncCustomSelect(wrapper) {
    var native = wrapper.querySelector('select.aico-preorder2-select-native');
  ...
  function closeDropdown(wrapper) {
    ...
    wrapper.classList.remove('aico-preorder2-custom-select-open');
  }
  ...
  function buildOptionRow(wrapper, native, opt) {
    ...
    row.className = 'aico-preorder2-custom-select-option';
  ...
  function openDropdown(wrapper) {
    ...
    wrapper.classList.add('aico-preorder2-custom-select-open');
  }
  ...
  function initCustomSelect(root) {
    ...
    wrappers.forEach(function (wrapper) {
      ...
      trigger.addEventListener('click', function (event) {
        ...
        var isOpen = wrapper.classList.contains('aico-preorder2-custom-select-open');
        if (isOpen) {
          closeDropdown(wrapper);
        } else {
  ...
  function initPreorderPage() {
    ...
    // date. It's server-rendered into data-aico-preorder2-deadline when known,
    ...
    var serverDeadline = root.getAttribute('data-aico-preorder2-deadline');
    ...
    var userId = root.getAttribute('data-aico-preorder2-user-id') || 'anon';
    var contextUrl = root.getAttribute('data-aico-preorder2-context-url');
    ...
    function applyStoredSelections() {
      ...
      Object.keys(stored).forEach(function (key) {
        ...
        var select = root.querySelector('[data-aico-preorder2-select="' + key + '"]');
    ...
    var form = root.querySelector('[data-aico-preorder2-filters]');
    var tokenInput = form ? form.querySelector('input[name="_token"]') : null;

    var selects = root.querySelectorAll('[data-aico-preorder2-select]');
    selects.forEach(function (select) {
      select.addEventListener('change', function () {
        var attributeKey = select.getAttribute('data-aico-preorder2-select');
    ...
    var viewButtons = root.querySelectorAll('[data-aico-preorder2-view]');
    function applyViewMode(value) {
      viewButtons.forEach(function (button) {
        var pressed = button.getAttribute('data-aico-preorder2-view') === value;
        button.classList.toggle('aico-preorder2-view-button-active', pressed);
        button.setAttribute('aria-pressed', pressed ? 'true' : 'false');
      });
      root.setAttribute('data-aico-preorder2-view-mode', value);
    }
    ...
    viewButtons.forEach(function (button) {
      button.addEventListener('click', function () {
        var next = button.getAttribute('data-aico-preorder2-view');
        if (!next) return;
        try {
  ...
  function getSelectValue(root, name) {
    var select = root.querySelector('[data-aico-preorder2-select="' + name + '"]');
  ...
  function setupPreorderWorkflow(root, opts) {
    ...
    var catalog = null;
    var cartCtrl = null;
    var isManager = root.getAttribute('data-aico-preorder2-is-manager') === '1';
    var cartEnabled = root.getAttribute('data-aico-preorder2-enable-cart') === '1';
    var defaultDebtorId =
      root.getAttribute('data-aico-preorder2-selected-debtor-id') || '';
    var currentDebtorId = defaultDebtorId;
    var b2bsUrl = root.getAttribute('data-aico-preorder2-b2bs-url');
    var b2bData = [];
    var mainEl = root.querySelector('[data-aico-preorder2-main]');
    var checkoutEl = root.querySelector('[data-aico-preorder2-checkout]');
    var catalogToolsEl = root.querySelector('[data-aico-preorder2-catalog-tools]');
    var filtersWrapEl = root.querySelector('[data-aico-preorder2-filters-wrap]');
    var resolvingEl = root.querySelector('[data-aico-preorder2-resolving]');
    var heroEl = root.querySelector('[data-aico-preorder2-hero]');
    var heroImgEl = root.querySelector('[data-aico-preorder2-hero-img]');
    var heroTitleEl = root.querySelector('[data-aico-preorder2-hero-title]');
    var heroBookEl = root.querySelector('[data-aico-preorder2-hero-book]');
    var noSessionEl = root.querySelector('[data-aico-preorder2-no-session]');
    ...
    var promptEl = root.querySelector('[data-aico-preorder2-prompt]');
    var promptTitleEl = promptEl ? promptEl.querySelector('.aico-preorder2-prompt-title') : null;
    var reopenNoticeEl = root.querySelector('[data-aico-preorder2-reopen-notice]');
    var reopenAckBtn = root.querySelector('[data-aico-preorder2-reopen-ack]');
    ...
    var eligibilityNoticeEl = root.querySelector('[data-aico-preorder2-eligibility-notice]');
    var eligibilityListEl = root.querySelector('[data-aico-preorder2-eligibility-list]');
    var eligibilityUrl = root.getAttribute('data-aico-preorder2-eligibility-url');
    ...
    var addressesUrl = root.getAttribute('data-aico-preorder2-addresses-url');
    var contextUrl = opts.contextUrl;
    var tokenInput = opts.tokenInput;
    var storageKey = opts.storageKey;
    var loadingEl = root.querySelector('[data-aico-preorder2-loading]');
    var errorEl = root.querySelector('[data-aico-preorder2-error]');
    var catalogEl = root.querySelector('[data-aico-preorder2-catalog]');
    var dateSelectEl = root.querySelector('[data-aico-preorder2-date-select]');
    var submitBtn = root.querySelector('[data-aico-preorder2-submit]');
    ...
    var summaryLines = root.querySelector('[data-aico-preorder2-summary-lines]');
    var summaryTotal = root.querySelector('[data-aico-preorder2-summary-total]');
    var summarySubtotalRow = root.querySelector('[data-aico-preorder2-summary-subtotal-row]');
    var summaryDiscountRow = root.querySelector('[data-aico-preorder2-summary-discount-row]');
    var summaryBilling = root.querySelector('[data-aico-preorder2-summary-billing]');
    var summaryDelivery = root.querySelector('[data-aico-preorder2-summary-delivery]');
    ...
    var cartPanel = root.querySelector('[data-aico-preorder2-cart-panel]');
    var cartPanelTab = cartPanel && cartPanel.querySelector('[data-aico-preorder2-cart-panel-tab]');
    var cartPanelTabQty = cartPanel && cartPanel.querySelector('[data-aico-preorder2-cart-panel-tab-qty]');
    var cartPanelPlaced = cartPanel && cartPanel.querySelector('[data-aico-preorder2-cart-panel-placed]');
    var cartPanelTotalQty = cartPanel && cartPanel.querySelector('[data-aico-preorder2-cart-panel-total-qty]');
    var cartPanelDiscount = cartPanel && cartPanel.querySelector('[data-aico-preorder2-cart-panel-discount]');
    var cartPanelNextTier = cartPanel && cartPanel.querySelector('[data-aico-preorder2-cart-panel-next-tier]');
    var cartPanelSubtotal = cartPanel && cartPanel.querySelector('[data-aico-preorder2-cart-panel-subtotal]');
    var cartPanelSideBtn = cartPanel && cartPanel.querySelector('[data-aico-preorder2-cart-panel-side]');
    var cartPanelCloseBtn = cartPanel && cartPanel.querySelector('[data-aico-preorder2-cart-panel-close]');
    ...
    if (cartPanel) {
      ...
      var legendWrap = cartPanel.querySelector('[data-aico-preorder2-cart-panel-legend]');
      wireCartPanelPopover(
        legendWrap,
        legendWrap && legendWrap.querySelector('[data-aico-preorder2-cart-panel-legend-trigger]')
      );
    ...
    function updateCartPanel(totalQty, grandTotal, discPct, discounts, currency, discountQty) {
      if (!cartPanel) return;
      var pcs = root.getAttribute('data-aico-preorder2-copy-pcs') || 'STK';
    ...
    var summaryStatus = root.querySelector('[data-aico-preorder2-summary-status]');
    var searchInput = root.querySelector('[data-aico-preorder2-search]');
    var termsCheckbox = root.querySelector('[data-aico-preorder2-terms]');
    var notesEl = root.querySelector('[data-aico-preorder2-notes]');
    var hasDirty = false;
    ...
    function buyerCountry() {
      var select = root.querySelector('[data-aico-preorder2-select="delivery_address_id"]');
    ...
    function updatePrompt() {
      ...
      var msg =
        root.getAttribute('data-aico-preorder2-copy-select-addresses') ||
    ...
    function renderEligibilityNotice() {
      ...
      eligibilityProblems.forEach(function (problem) {
        ...
        li.className = 'aico-preorder2-eligibility-notice-item';
    ...
    function applyNoSessionVisibility() {
      ...
      if (reopenNoticeEl) {
        ...
        reopenNoticeEl.classList.remove('aico-preorder2-reopen-notice-loading');
      }
    ...
    function applyResolvingVisibility() {
      ...
      if (reopenNoticeEl) {
        reopenNoticeEl.classList.add('aico-preorder2-reopen-notice-loading');
    ...
    function applyReopenVisibility(cartResolved) {
      ...
      if (resolvingEl) resolvingEl.hidden = true;
      if (reopenNoticeEl) reopenNoticeEl.classList.remove('aico-preorder2-reopen-notice-loading');
    ...
    function setB2bLoading(isLoading) {
      var field = root.querySelector('[data-aico-preorder2-b2b-field]');
      if (field) field.classList.toggle('aico-preorder2-field--loading', !!isLoading);
    }
    ...
    function setAddressLoading(isLoading) {
      ['billing_address_id', 'delivery_address_id'].forEach(function (name) {
        var select = root.querySelector('[data-aico-preorder2-select="' + name + '"]');
        var field = select && select.closest('.aico-preorder2-field');
        if (field) field.classList.toggle('aico-preorder2-field--loading', !!isLoading);
      });
    }
    ...
    function applyB2bSelection() {
      ...
      var debtorField = root.querySelector('[data-aico-preorder2-debtor-field]');
      var match = null;
    ...
    function setB2bLoadingMore(isLoading) {
      ...
      var sentinel = panel ? panel.querySelector('[data-aico-preorder2-b2b-more]') : null;
    ...
    function fetchB2bPage(pageNumber, options) {
      ...
      fetch(url.toString(), {
        ...
        .then(function (json) {
          ...
          if (opts.keepOpen) {
            var wrapper = root.querySelector('[data-aico-preorder2-b2b-field] [data-aico-custom-select]');
            if (wrapper) {
              wrapper.classList.add('aico-preorder2-custom-select-open');
              var panel = wrapper.querySelector('[data-aico-custom-select-dropdown]');
              if (panel) panel.hidden = false;
            }
            var input = root.querySelector('[data-aico-preorder2-b2b-search]');
    ...
    function renderDateSelect(dates) {
      ...
      function row(value, text, checked) {
        return (
          '<button type="button" role="option" class="aico-preorder2-custom-select-option aico-preorder2-date-option' +
          (checked ? ' is-checked' : '') +
          '" data-date-value="' + escapeHtml(value) + '" aria-selected="' + (checked ? 'true' : 'false') + '">' +
          '<span class="aico-preorder2-date-check" aria-hidden="true"></span>' +
    ...
    function updateSummary() {
      ...
      var exclVat =
        root.getAttribute('data-aico-preorder2-copy-excl-vat') || 'excl. VAT';
      var pcsLabel = root.getAttribute('data-aico-preorder2-copy-pcs') || 'STK';
      ...
      if (summaryLines && dates.length) {
        summaryLines.innerHTML = dates
          .map(function (dateLabel) {
            ...
            var disabled = qty <= 0;
            return (
              '<div class="aico-preorder2-summary-date-row' +
              (disabled ? ' is-disabled' : '') +
              '">' +
              '<p class="aico-preorder2-summary-date-title">' +
              dateLabel +
              '</p>' +
              '<div class="aico-preorder2-summary-date-body">' +
              '<span>' +
              (qty > 0
      ...
      var productsLabel =
        root.getAttribute('data-aico-preorder2-copy-subtotal') || 'Subtotal';

      if (summarySubtotalRow) {
        summarySubtotalRow.innerHTML =
          '<span class="aico-preorder2-summary-label">' +
          productsLabel +
          '</span>' +
          '<span><span class="aico-preorder2-badge">' +
          totalQty +
          '</span>' +
      ...
      if (summaryDiscountRow) {
        if (discPct > 0) {
          ...
          summaryDiscountRow.innerHTML =
            '<span class="aico-preorder2-summary-label">' +
            (root.getAttribute('data-aico-preorder2-copy-discount') || 'Discount') +
            '</span>' +
            '<span><span class="aico-preorder2-badge">' +
            discPct +
            '%</span>' +
          ...
          summaryDiscountRow.innerHTML =
            ...
            (root.getAttribute('data-aico-preorder2-copy-discount') || 'Discount') +
            '</span>' +
            '<span><span class="aico-preorder2-badge">0%</span>' +
      ...
      if (summaryTotal) {
        summaryTotal.innerHTML =
          '<span>' +
          (root.getAttribute('data-aico-preorder2-copy-total') || 'TOTAL AMOUNT') +
          '</span>' +
          '<span><span class="aico-preorder2-grand-amount">' +
          formatMoney(grandTotal, currency, '') +
          '</span>' +
          '<span class="aico-preorder2-grand-vat">' +
          exclVat +
          '</span></span>';
      }

      var saving = cartCtrl && cartCtrl.saving;
      if (submitBtn && !submitBtn.classList.contains('aico-preorder2-btn--saving')) {
    ...
    function fetchSession(signal) {
      var url = root.getAttribute('data-aico-preorder2-session-url');
    ...
    function readEmbeddedSession() {
      var el = document.querySelector('[data-aico-preorder2-session-json]');
      if (!el) return null;
      try {
    ...
    function applyHero(session) {
      ...
      if (heroEl) {
        ...
        if (from && to) {
          heroEl.style.setProperty('--aico-preorder2-grad-from', from);
          heroEl.style.setProperty('--aico-preorder2-grad-to', to);
        } else {
          heroEl.style.removeProperty('--aico-preorder2-grad-from');
          heroEl.style.removeProperty('--aico-preorder2-grad-to');
        }
      }
    }
    ...
    function getFlyerCopy() {
      return {
        total:
          root.getAttribute('data-aico-preorder2-copy-flyer-total') || 'Total',
        pcs: root.getAttribute('data-aico-preorder2-copy-pcs') || 'STK',
      };
    }
    ...
    function updateFlyerCardTotals(card, quantity) {
      var totalEl = card.querySelector('[data-aico-preorder2-flyer-total]');
      if (totalEl) {
    ...
    function bindFlyerSliders() {
      var grid = root.querySelector('[data-aico-preorder2-flyers-grid]');
      if (!grid) return;
      grid.querySelectorAll('[data-aico-preorder2-flyer-range]').forEach(function (range) {
        ...
        range.addEventListener('input', function () {
          syncTrack();
          var card = range.closest('[data-aico-preorder2-flyer-card]');
    ...
    function renderFlyers(flyers) {
      var section = root.querySelector('[data-aico-preorder2-flyers]');
      var grid = root.querySelector('[data-aico-preorder2-flyers-grid]');
      ...
      var existingCards = grid.querySelectorAll('[data-aico-preorder2-flyer-card]');
      var sameFlyerSet =
        existingCards.length === flyers.length &&
        flyers.every(function (f) {
          return grid.querySelector('[data-aico-preorder2-flyer-card][data-flyer-id="' + f.id + '"]');
        });
      if (sameFlyerSet) {
        flyers.forEach(function (f) {
          var card = grid.querySelector('[data-aico-preorder2-flyer-card][data-flyer-id="' + f.id + '"]');
          var range = card && card.querySelector('[data-aico-preorder2-flyer-range]');
          if (!range) return;
      ...
      grid.innerHTML = flyers
        .map(function (f) {
          ...
          return (
            '<article class="aico-preorder2-flyer-card" data-aico-preorder2-flyer-card data-flyer-id="' +
            flyerId +
            '">' +
            '<header class="aico-preorder2-flyer-card__head">' +
            '<h4 class="aico-preorder2-flyer-card__title">' +
            escapeHtml(title) +
            '</h4>' +
            '<p class="aico-preorder2-flyer-card__total" data-aico-preorder2-flyer-total>' +
            escapeHtml(copy.total + ': ' + totalPcs + ' ' + copy.pcs) +
            '</p>' +
            '</header>' +
            (sku
              ? '<p class="aico-preorder2-flyer-card__sku">' + escapeHtml(sku) + '</p>'
              : '') +
            '<div class="aico-preorder2-flyer-slider">' +
            '<input type="range" class="aico-preorder2-flyer-range" data-aico-preorder2-flyer-range data-flyer-id="' +
            flyerId +
            ...
            escapeHtml(title) +
            '">' +
            '<div class="aico-preorder2-flyer-marks">' +
            marks +
            '</div>' +
            '</div></article>'
          );
    ...
    catalog = new window.AicoPreorderCatalog.CatalogController({
      root: root,
      productsUrl: root.getAttribute('data-aico-preorder2-products-url'),
      catalogEl: catalogEl,
      loadingEl: loadingEl,
      errorEl: errorEl,
      getCopy: function () {
        return {
          empty: root.getAttribute('data-aico-preorder2-copy-empty'),
          productsHeading: root.getAttribute('data-aico-preorder2-copy-products-heading'),
          rowTotal: root.getAttribute('data-aico-preorder2-copy-row-total'),
          productTotal: root.getAttribute('data-aico-preorder2-copy-product-total'),
          pcs: root.getAttribute('data-aico-preorder2-copy-pcs'),
        };
      },
    ...
    if (searchInput) {
      ...
      searchSpinner.className = 'aico-preorder2-search-spinner';
      searchSpinner.setAttribute('aria-hidden', 'true');
      searchSpinner.hidden = true;
      var searchControl =
        searchInput.closest('.aico-preorder2-field-control') || searchInput.parentElement;
      if (searchControl) {
        searchControl.classList.add('aico-preorder2-field-control--searchable');
    ...
    if (cartEnabled) {
      cartCtrl = new window.AicoPreorderCart.CartController({
        cartUrl: root.getAttribute('data-aico-preorder2-cart-url'),
        addUrl: root.getAttribute('data-aico-preorder2-cart-add-url'),
        changeUrl: root.getAttribute('data-aico-preorder2-cart-change-url'),
        clearUrl: root.getAttribute('data-aico-preorder2-cart-clear-url'),
        submitUrl: root.getAttribute('data-aico-preorder2-cart-submit-url'),
        ...
        onError: function (code, payload) {
          ...
          if (code === 'committed_quantity') {
            summaryStatus.textContent =
              root.getAttribute('data-aico-preorder2-copy-committed') ||
              'Some lines are already confirmed and cannot be lowered.';
          } else {
            summaryStatus.textContent =
              root.getAttribute('data-aico-preorder2-copy-error') || 'Error';
          }
        },
      });
    }

    root.querySelectorAll('[data-aico-preorder2-select]').forEach(function (select) {
      select.addEventListener('change', function () {
        var key = select.getAttribute('data-aico-preorder2-select');
        if (key === 'b2b_id') {
          applyB2bSelection();
          return;
        }
    ...
    function setSubmitSaving(isSaving) {
      ...
      if (isSaving) {
        ...
        submitBtn.classList.add('aico-preorder2-btn--saving');
        submitBtn.setAttribute('aria-busy', 'true');
        var savingCopy =
          root.getAttribute('data-aico-preorder2-copy-saving') || 'Saving cart…';
        submitBtn.innerHTML =
          '<span class="aico-preorder2-btn-spinner" aria-hidden="true"></span>' +
          '<span>' + escapeHtml(savingCopy) + '</span>';
      } else {
        submitBtn.classList.remove('aico-preorder2-btn--saving');
    ...
    function flagTermsRequired() {
      var wrap = termsCheckbox
        ? termsCheckbox.closest('.aico-preorder2-terms')
        : null;
      if (wrap) {
        wrap.classList.add('aico-preorder2-terms--required');
        // restart the shake animation on each click
        wrap.classList.remove('aico-preorder2-terms--shake');
        void wrap.offsetWidth;
        wrap.classList.add('aico-preorder2-terms--shake');
    ...
    function revealInlineConfirmation(cartId) {
      var panel = root.querySelector('[data-aico-preorder2-confirmation]');
      if (!panel || !window.AicoPreorderConfirmation) {
        // No confirmation module/panel — fall back to the old status line.
        if (summaryStatus) {
          summaryStatus.hidden = false;
          summaryStatus.textContent =
            root.getAttribute('data-aico-preorder2-copy-submitted') || 'Preorder placed';
        }
        return;
      }
      if (checkoutEl) checkoutEl.hidden = true;
      ...
      submitBtn.addEventListener('click', function () {
        ...
        cartCtrl
          ...
          .then(function () {
            ...
            var thankYou = root.getAttribute('data-aico-preorder2-thank-you-url');
            if (thankYou) {
              // Redirect immediately — the submit is queued, not finished. Carry
              // the cart id so the thank-you page can confirm against it
              // (preorder2-confirmation.js reads ?cart=). That page shows the
    ...
    if (termsCheckbox) {
      termsCheckbox.addEventListener('change', function () {
        if (termsCheckbox.checked) {
          var wrap = termsCheckbox.closest('.aico-preorder2-terms');
          if (wrap) {
            wrap.classList.remove(
              'aico-preorder2-terms--required',
              'aico-preorder2-terms--shake',
            );
          }
        }
        updateSummary();
    ...
    if (reopenAckBtn) {
      reopenAckBtn.addEventListener('click', function () {
        ...
        var finish = function () {
          ...
          noticeEl.classList.remove('aico-preorder2-reopen-notice-leaving');
          applyReopenVisibility(true);
        };
        noticeEl.classList.add('aico-preorder2-reopen-notice-leaving');
    ...
    var galleryEl = root.querySelector('[data-aico-preorder2-gallery]');
    if (galleryEl && catalogEl) {
      var productImagesUrl = root.getAttribute('data-aico-preorder2-product-images-url');
      var galMainImg = galleryEl.querySelector('[data-aico-preorder2-gallery-main]');
      var galTitle = galleryEl.querySelector('[data-aico-preorder2-gallery-title]');
      var galThumbs = galleryEl.querySelector('[data-aico-preorder2-gallery-thumbs]');
      var galEmpty = galleryEl.querySelector('[data-aico-preorder2-gallery-empty]');
      var galPrev = galleryEl.querySelector('[data-aico-preorder2-gallery-prev]');
      var galNext = galleryEl.querySelector('[data-aico-preorder2-gallery-next]');
      var galImages = [];
      var galIndex = 0;
      var galReqId = 0;
      ...
      function galShow(i) {
        ...
        galThumbs
          .querySelectorAll('.aico-preorder2-gallery-thumb')
      ...
      function galRender(data, fallbackImg, fallbackTitle) {
        ...
        galImages.forEach(function (img, idx) {
          ...
          b.type = 'button';
          b.className = 'aico-preorder2-gallery-thumb';
      ...
      function galOpen(productId, placeholderImg, placeholderTitle) {
        ...
        document.documentElement.classList.add('aico-preorder2-gallery-open');
        galImages = [];
        galEmpty.hidden = true;
        galPrev.hidden = true;
        galNext.hidden = true;
        ...
        for (var s = 0; s < 5; s++) {
          ...
          skeleton.className =
            'aico-preorder2-gallery-thumb aico-preorder2-gallery-thumb--skeleton';
      ...
      function galClose() {
        ...
        document.documentElement.classList.remove('aico-preorder2-gallery-open');
      }

      catalogEl.addEventListener('click', function (event) {
        var media = event.target.closest
          ? event.target.closest('.aico-preorder2-product-media')
          : null;
        if (!media || !catalogEl.contains(media)) return;
        var card = media.closest('[data-aico-preorder2-product-id]');
        if (!card) return;
        event.preventDefault();
        var mediaImg = media.querySelector('img');
        var titleEl = card.querySelector('.aico-preorder2-product-title');
        galOpen(
          card.getAttribute('data-aico-preorder2-product-id'),
      ...
      galleryEl
        .querySelectorAll('[data-aico-preorder2-gallery-close]')
        .forEach(function (el) {
    ...
    var b2bTriggerEl = root.querySelector(
      '[data-aico-preorder2-b2b-field] [data-aico-custom-select-trigger]',
    );
    if (b2bTriggerEl) {
    ...
    var b2bSearchInput = root.querySelector('[data-aico-preorder2-b2b-search]');
    if (b2bSearchInput) {
      var b2bSearchTimer;
  ...
  // + data-attribute on the stable .aico-preorder2 root (NOT on re-rendered
  ...
  function initPreorderDisplayOptions() {
    var root = document.querySelector('[data-aico-preorder2]');
    if (!root) return;
    var field = root.querySelector('[data-aico-preorder2-display]');
    if (!field) return;
    var trigger = field.querySelector('[data-aico-preorder2-display-trigger]');
    var panel = field.querySelector('[data-aico-preorder2-display-panel]');
    var stripInput = field.querySelector('[data-aico-preorder2-display-strip]');
    var sizeInputs = Array.prototype.slice.call(
      field.querySelectorAll('[data-aico-preorder2-display-sizes]'),
    );

    var prefs = readDisplayPrefs();

    function apply() {
      root.classList.toggle('aico-preorder2--no-strip', !prefs.strip);
