 ...
 *
 * Renders, into a `[data-aico-preorder2-confirmation]` panel:
 *   - a success header,
...
(function (global) {
  ...
  Confirmation.prototype.renderPending = function () {
    this.root.classList.add('aico-preorder2-confirmation--processing');
  ...
  Confirmation.prototype.clearPending = function () {
    this.root.classList.remove('aico-preorder2-confirmation--processing');
  ...
  Confirmation.prototype.renderError = function () {
    ...
    this.clearPending();
    this.root.hidden = false;
    this.root.classList.add('aico-preorder2-confirmation--error');
  ...
  Confirmation.prototype.renderMissing = function () {
    this.clearPending();
    this.root.hidden = false;
    this.root.classList.add('aico-preorder2-confirmation--missing');
  ...
  Confirmation.prototype._renderProgress = function (data) {
    if (this._errored || !data) return;
    if (!this.root.classList.contains('aico-preorder2-confirmation--processing')) return;
  ...
  Confirmation.prototype._swapToState = function (render) {
    ...
    var processing = root.classList.contains('aico-preorder2-confirmation--processing');
    ...
    this._swapping = true;
    this._pendingSwapRender = render;
    root.classList.add('aico-preorder2-confirmation--swap-out');
    var self = this;
    setTimeout(function () {
      var run = self._pendingSwapRender;
      self._pendingSwapRender = null;
      if (run) run();
      root.classList.remove('aico-preorder2-confirmation--swap-out');
      root.classList.add('aico-preorder2-confirmation--swap-in');
      setTimeout(function () {
        root.classList.remove('aico-preorder2-confirmation--swap-in');
  ...
  Confirmation.prototype.renderDetails = function (confirmation) {
    this.root.classList.remove('aico-preorder2-confirmation--error');
    this.clearPending();
    this.root.hidden = false;
  ...
  Confirmation.prototype.renderPerDate = function (rows) {
    ...
    container.innerHTML = rows
      .map(function (row) {
        return (
          '<div class="aico-preorder2-confirmation-date-row">' +
          '<span class="aico-preorder2-confirmation-date">' +
          escapeHtml(formatDate(row.delivery_date)) +
          '</span>' +
          '<span class="aico-preorder2-confirmation-date-number">' +
          escapeHtml(row.order_number || '') +
          '</span>' +
          '<span class="aico-preorder2-confirmation-date-qty">' +
          escapeHtml(String(row.quantity || 0) + ' ' + qtyLabel) +
          '</span>' +
          '<span class="aico-preorder2-confirmation-date-total">' +
  ...
  Confirmation.prototype.setPdfState = function (state) {
    ...
    if (state === 'generating') {
      area.innerHTML =
        '<span class="aico-preorder2-confirmation-pdf-spinner" aria-hidden="true"></span>' +
        '<span>' +
  ...
  Confirmation.prototype.showPdfLink = function (fileUrl, fileName) {
    ...
    area.innerHTML =
      '<a class="aico-preorder2-confirmation-pdf-link aico-btn aico-btn-primary" href="' +
      escapeHtml(fileUrl) +
  ...
  function init() {
    var root = document.querySelector('[data-aico-preorder2-confirmation]');
    if (!root) return;
