 ...
 *
 * The real controls are NEVER moved. preorder-page.js resolves its bindings with
 * live `root.querySelector(...)` calls, so reparenting a field out of
 * `[data-aico-preorder]` would silently break it. Everything here is a MIRROR
 * that writes to the real control and dispatches the event preorder-page.js
 * already listens for:
 *
 ...
 * shared bundles /preorder and /preorder2 both load.
 */
(function () {
  'use strict';
  ...
  function init() {
    ...
    var toolsEl = root.querySelector('[data-aico-preorder-catalog-tools]');
    ...
    // preorder-page.js writes the hero title only once the session resolves, so
    ...
    var realSearch = root.querySelector('[data-aico-preorder-search]');
    ...
    var realDateSelect = root.querySelector('[data-aico-preorder-date-select]');
    ...
    var realSize = root.querySelector('[data-aico-preorder-select="size_region"]');
    ...
    function renderSizeRows() {
      ...
      Array.prototype.forEach.call(realSize.options, function (option) {
        ...
        row.addEventListener('click', function (event) {
          ...
          // `change` is what preorder-page.js binds (persist + re-render) and what
    ...
    function update() {
      ...
      // Only dock the tools once they actually exist on screen — they stay hidden
      // until a session + addresses are chosen.
      var toolsVisible = !!toolsEl && toolsEl.getBoundingClientRect().height > 0;
      var showTools = toolsVisible && toolsEl.getBoundingClientRect().bottom <= headerBottom;
