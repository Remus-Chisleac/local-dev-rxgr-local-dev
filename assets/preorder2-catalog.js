...
(function (global) {
  ...
  function renderQtyInput(opts) {
    ...
    var classes = 'aico-preorder2-qty-box';
    if (filled) classes += ' aico-preorder2-qty-box--filled';
    if (committedStyle) classes += ' aico-preorder2-qty-box--committed';
    if (boxDisabled) classes += ' aico-preorder2-qty-box--disabled';
    return (
      '<span class="' +
      classes +
      '">' +
      '<span class="aico-preorder2-qty-box__size" aria-hidden="true">' +
      sizeLabelHtml +
      '</span>' +
      '<span class="aico-preorder2-qty-box__inner">' +
      '<input type="number" class="aico-preorder2-qty-input" step="1"' +
      minAttr +
      ' value="' +
      displayVal +
      '"' +
      maxAttr +
      inputDisabledAttr +
      ' data-aico-preorder2-qty ' +
      attrs +
      ' aria-label="' +
      escapeHtml(ariaLabel) +
      '">' +
      '<span class="aico-preorder2-qty-box__spinners">' +
      '<button type="button" class="aico-preorder2-qty-box__step" data-aico-preorder2-qty-step="1" tabindex="-1"' +
  ...
  CatalogController.prototype.render = function () {
    ...
    if (
      ...
      activeEl.hasAttribute('data-aico-preorder2-qty') &&
    ...
    if (!this.products.length && !this.loading) {
      this.catalogEl.innerHTML =
        '<p class="aico-preorder2-catalog-empty">' +
    ...
    this.catalogEl.style.setProperty('--aico-preorder2-date-w', '5.5rem');
    this.catalogEl.style.setProperty('--aico-preorder2-total-w', '3.5rem');
    ...
    groupOrder.forEach(function (groupName) {
      ...
      groupsHtml +=
        '<section class="aico-preorder2-group' +
        (collapsed ? ' aico-preorder2-group-collapsed' : '') +
        (groupLoading ? ' aico-preorder2-group-loading' : '') +
        (!items.length && !groupLoading ? ' aico-preorder2-group-empty' : '') +
        '" data-aico-preorder2-group style="--aico-preorder2-cell-w:' +
        cellW +
        '">';
      groupsHtml += '<header class="aico-preorder2-group-head">';
      groupsHtml += '<div class="aico-preorder2-group-header-row">';
      // The title is a toggle button so the whole category can be collapsed.
      groupsHtml +=
        '<h3 class="aico-preorder2-group-title">' +
        '<button type="button" class="aico-preorder2-group-toggle" data-aico-preorder2-group-toggle ' +
        'data-group-name="' + escapeHtml(group.optionGroupName) + '" aria-expanded="' +
        (collapsed ? 'false' : 'true') + '">' +
        '<svg class="aico-preorder2-group-chevron" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><polyline points="6 9 12 15 18 9"></polyline></svg>' +
        '<span class="aico-preorder2-group-name">' + escapeHtml(group.optionGroupName) + '</span>' +
        '</button>' +
        '</h3>';
      ...
      groupsHtml +=
        '<div class="aico-preorder2-matrix-scroll aico-preorder2-matrix-scroll--header" data-aico-preorder2-matrix-scroll aria-hidden="true">' +
        '<div class="aico-preorder2-matrix-track">';
      ...
      groupsHtml +=
        ...
        '<span class="aico-preorder2-matrix-total-head-label">' +
        escapeHtml(copy.productTotal || 'Total') +
        '</span> ' +
        '<span class="aico-preorder2-group-ordered-total" data-aico-preorder2-group-ordered-total>' +
      ...
      groupsHtml += '<div class="aico-preorder2-group-body"><div class="aico-preorder2-group-body-inner">';
      ...
      if (groupLoading) {
        ...
        groupsHtml +=
          '<p class="aico-preorder2-group-empty-note">' +
          escapeHtml(copy.empty || 'No products.') +
          '</p>';
      }
      groupsHtml += '</div></div>';
    ...
    var countBadge = searching
      ? '<span class="aico-preorder2-products-count" data-aico-preorder2-products-count>' +
        matchedCount + '/' + totalCount + '</span>'
      : '';
    var html =
      '<div class="aico-preorder2-products-heading">' +
    ...
    if (!groupsHtml && searching) {
      ...
      html +=
        '<p class="aico-preorder2-catalog-empty aico-preorder2-no-matches">' +
        escapeHtml(copy.empty || 'No products.') +
  ...
  CatalogController.prototype.restoreFocusedCell = function (cell) {
    if (!this.catalogEl) return;
    var inputs = this.catalogEl.querySelectorAll('[data-aico-preorder2-qty]');
    for (var i = 0; i < inputs.length; i++) {
      ...
      if (
        ...
        var box = input.closest('.aico-preorder2-qty-box');
        if (box) box.classList.add('aico-preorder2-qty-box--editing');
        try {
  ...
  CatalogController.prototype.bindGroupToggles = function () {
    ...
    if (!this.catalogEl) return;
    this.catalogEl
      .querySelectorAll('[data-aico-preorder2-group-toggle]')
      .forEach(function (btn) {
        btn.addEventListener('click', function () {
          var name = btn.getAttribute('data-group-name');
          var section = btn.closest('[data-aico-preorder2-group]');
          var collapsed = !self.collapsedGroups[name];
          self.collapsedGroups[name] = collapsed;
          if (section) {
            section.classList.toggle('aico-preorder2-group-collapsed', collapsed);
          }
  ...
  CatalogController.prototype.refreshProduct = function (productId) {
    ...
    var card = this.catalogEl.querySelector(
      '[data-aico-preorder2-product-id="' + productId + '"]',
    );
    if (!card) return;
    var self = this;
    var session = this.getSession() || {};
    ...
    card.querySelectorAll('[data-aico-preorder2-qty]').forEach(function (input) {
      ...
      var box = input.closest('.aico-preorder2-qty-box');
      var incBtn = box ? box.querySelector('[data-aico-preorder2-qty-step="1"]') : null;
      var decBtn = box ? box.querySelector('[data-aico-preorder2-qty-step="-1"]') : null;
      ...
      if (box) {
        box.classList.toggle('aico-preorder2-qty-box--disabled', cell.boxDisabled);
        box.classList.toggle('aico-preorder2-qty-box--filled', cell.filled);
        box.classList.toggle('aico-preorder2-qty-box--committed', cell.committedStyle);
      }
    });

    card.querySelectorAll('.aico-preorder2-matrix-row').forEach(function (row) {
      var dateEl = row.querySelector('.aico-preorder2-matrix-date');
      var totalEl = row.querySelector('.aico-preorder2-matrix-row-total strong');
      if (dateEl && totalEl) {
        totalEl.textContent = rowTotalForDate(productId, dateEl.textContent.trim());
      }
    });

    var totalStrong = card.querySelector('.aico-preorder2-product-total strong');
    if (totalStrong) {
    ...
    this.refreshGroupOrderedTotal(card.closest('[data-aico-preorder2-group]'));
  };
  ...
  CatalogController.prototype.refreshGroupOrderedTotal = function (sectionEl) {
    if (!sectionEl) return;
    var totalEl = sectionEl.querySelector('[data-aico-preorder2-group-ordered-total]');
    if (!totalEl) return;
    var copy = this.getCopy();
    var sum = 0;
    sectionEl.querySelectorAll('[data-aico-preorder2-product-id]').forEach(function (card) {
      sum += productTotal(parseInt(card.getAttribute('data-aico-preorder2-product-id'), 10));
    });
  ...
  CatalogController.prototype.renderSkeletonCard = function () {
    return (
      '<article class="aico-preorder2-product-card aico-preorder2-product-card--skeleton" aria-hidden="true">' +
      '<div class="aico-preorder2-product-grid">' +
      '<div class="aico-preorder2-product-aside">' +
      '<div class="aico-preorder2-product-media"><span class="aico-skeleton aico-skel-pulse aico-preorder2-skel-media"></span></div>' +
      '<div class="aico-preorder2-product-info">' +
      '<span class="aico-skeleton aico-skel-pulse aico-preorder2-skel-line aico-preorder2-skel-line--title"></span>' +
      '<span class="aico-skeleton aico-skel-pulse aico-preorder2-skel-line aico-preorder2-skel-line--sku"></span>' +
      '<span class="aico-skeleton aico-skel-pulse aico-preorder2-skel-line aico-preorder2-skel-line--price"></span>' +
      '</div></div>' +
      '<div class="aico-preorder2-product-rows">' +
      '<span class="aico-skeleton aico-skel-pulse aico-preorder2-skel-row"></span>' +
      '<span class="aico-skeleton aico-skel-pulse aico-preorder2-skel-row"></span>' +
      '</div>' +
      '</div></article>'
    );
  ...
  CatalogController.prototype.renderProductCard = function (
    ...
    if (price != null && price >= 0) {
      ...
      html += '<p class="aico-preorder2-product-price">';
      if (comparePrice != null) {
        html +=
          '<span class="aico-preorder2-product-price-was">' +
          escapeHtml(formatMoney(comparePrice, currency)) +
          '</span>';
      }
      html +=
        '<span class="aico-preorder2-product-price-now">' +
    ...
    dates.forEach(function (dateLabel, dateIdx) {
      ...
      sizeValues.forEach(function (optVal) {
        ...
        if (!variant) {
          html +=
            '<span class="aico-preorder2-qty-box aico-preorder2-qty-box--missing" aria-hidden="true">' +
            '<span class="aico-preorder2-qty-box__dash">-</span></span>';
          return;
        }
      ...
      html +=
        '<div class="aico-preorder2-matrix-row-total"><strong>' +
        rowTotal +
        '</strong></div>';
      html += '</div>';
    });
  ...
  CatalogController.prototype.bindQuantityInputs = function () {
    ...
    this.catalogEl.querySelectorAll('[data-aico-preorder2-qty]').forEach(function (input) {
      // Editing model:
      ...
      input.addEventListener('blur', function () {
        var box = input.closest('.aico-preorder2-qty-box');
        if (box) box.classList.remove('aico-preorder2-qty-box--editing');
    ...
    this.catalogEl.querySelectorAll('[data-aico-preorder2-qty-step]').forEach(function (btn) {
      btn.addEventListener('click', function (e) {
        e.preventDefault();
        if (btn.disabled) return;
        var control = btn.closest('.aico-preorder2-qty-box__inner');
        var input = control && control.querySelector('[data-aico-preorder2-qty]');
        if (!input || input.disabled) return;
        var step = parseInt(btn.getAttribute('data-aico-preorder2-qty-step'), 10) || 0;
